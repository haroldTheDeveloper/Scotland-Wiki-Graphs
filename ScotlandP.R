...
base <- ggplot(mydata) +
  ...
  scale_y_continuous(minor_breaks = seq(0,100,1),breaks = seq(0, 100, 5), expand = c(0,0), limits = c(0.1,45)) +
  ...
  geom_point(aes(Date, ALB, color="Alba"), alpha=0.6, size=S2)+
  scale_colour_manual(name="Parties", values=c("#005eb8", "#0087dc","#00b140","#e4003b","#faa61a","#f2d70c"))+
  ...
  geom_smooth(aes(Date, GRN),color="#00b140", fill = "lightblue3",method="loess", size=S, se=FALSE,n=kn, span = sSize)+
  geom_smooth(aes(Date, ALB),color="#005eb8", fill = "lightblue3",method="loess", size=S, se=FALSE,n=kn, span = sSize)+
